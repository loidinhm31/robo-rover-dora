...
fn main() -> Result<()> {
    ...
    println!(
        "Audio config: {}Hz, {} channels, {} samples per chunk",
        sample_rate, channels, chunk_size
    );
    ...
    let stream = device.build_input_stream(
        &config,
        move |data: &[f32], _: &_| write_audio_data(data, &mut producer),
        err_fn,
        None,
    )?;
    ...
    loop {
        match events.recv() {
            Some(Event::Input { id, .. }) if id.as_str() == "tick" => {
                ...
                while consumer.occupied_len() > 0 && audio_buffer.len() < chunk_size {
