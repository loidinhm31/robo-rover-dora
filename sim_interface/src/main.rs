...
use eyre::Result;
use robo_rover_lib::{ArmCommand, ArmCommandWithMetadata, ArmTelemetry, CompleteJointState, RoverCommand, RoverCommandWithMetadata, RoverTelemetry};
...
impl UnifiedRobotController {
    ...
    pub async fn send_complete_state(&self, state: &CompleteJointState) -> Result<()> {
        println!("Sending complete joint state to urdf-viz:");
        println!("  Rover wheels:");
        for i in 0..3 {
            println!("    {}: {:.3} rad ({:.1}°)",
        ...
        if response.status().is_success() {
            ...
            if result.is_ok {
                println!("✅ Joint positions sent successfully to urdf-viz!");
            } else {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🤖 Starting Unified Simulation Interface for LeKiwi Robot");
    println!("   - 3 Mecanum wheels for omnidirectional movement");
    println!("   - 6 DOF robotic arm");
    ...
    if let Err(e) = controller.home().await {
        eprintln!("⚠️  Warning: Failed to send home position to urdf-viz: {}", e);
        eprintln!("   Make sure urdf-viz is running with:");
        eprintln!("   urdf-viz model/LeKiwi.urdf");
    } else {
        println!("✅ Robot initialized at home position");
    }
            ...
            match event {
                Event::Input { id, data, .. } => {
                    ...
                    match id_str {
                                            ...
                                            if let Some(arm_cmd) = cmd_with_metadata.command {
                                                ...
                                                match arm_cmd {
                                                    ArmCommand::JointPosition { joint_angles, .. } => {
                                                        if joint_angles.len() >= 6 {
                                                            ...
                                                            match controller.update_arm(positions).await {
                                                                Ok(_) => {
                                                                    println!("✅ Arm positions sent successfully");
                                                                ...
                                                                Err(e) => {
                                                                    eprintln!("❌ Failed to send arm positions: {}", e);
                                                                }
                                                            }
                                                        } else {
                                                    ...
                                                    ArmCommand::Stop | ArmCommand::EmergencyStop => {
                                                        println!("🛑 Stop command received for arm");
                                                    }
                                                    _ => {
                                                        println!("⚠️  Unsupported arm command type for simulation");
                                                    }
                                                }
                                            }
                                        }
                        ...
                        "rover_command" => {
                            println!("Received rover command");
                                            ...
                                            match rover_cmd {
                                                RoverCommand::JointPositions { wheel1, wheel2, wheel3, .. } => {
                                                    match controller.update_rover(wheel1, wheel2, wheel3).await {
                                                        ...
                                                        Err(e) => {
                                                            eprintln!("Failed to send rover positions: {}", e);
                                                        }
                                                    }
                                                }
                                                RoverCommand::Stop { .. } => {
                                                    println!("Stop command received for rover");
                                                    // Keep current position, don't update
                                                    }
                                                    _ => {
                                                        println!("Rover command type will be converted to joint positions by rover-controller");
                                                    }
                                                }
                                        }
                                        Err(e) => {
                                            eprintln!("Failed to parse rover command: {}", e);
                                        }
                                    }
                                }
                            }
